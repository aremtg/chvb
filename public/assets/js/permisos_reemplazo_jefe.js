...
let permisosBandeja = JSON.parse(document.getElementById('listaBandeja').dataset.permisos || '[]');
const tieneFirmaGuardada = document.getElementById('listaBandeja').dataset.tieneFirmaGuardada === '1';

function esRolPara(p) {
    if (p.cedula_reemplazo === cedulaPropia && p.estado === 'por_firmar_reemplazo') return 'reemplazo';
    if (p.cedula_jefe === cedulaPropia && p.estado === 'por_firmar_jefe') return 'jefe';
    return null;
}
...
function render() {
    ...
    let html = '';
    if (pendientes.length === 0) html += '<p class="text-sm text-gray-400">No tienes permisos pendientes de firmar.</p>';
    ...
    if (historial.length > 0) {
        ...
        historial.forEach(p => {
            html += `<div class="bg-white rounded-xl shadow p-3 text-sm text-gray-600">${p.consecutivo} — ${p.nombre_empleado_snapshot} (${p.estado})</div>`;
        });
    }
    cont.innerHTML = html;
}
...
let accionActual = null, permisoIdActual = null, versionActual = null;
...
function abrirModalFirma(id, version, rol, accion) {
    permisoIdActual = id; versionActual = version; accionActual = accion;
    document.getElementById('errorBandeja').classList.add('hidden');
    document.getElementById('tituloModalFirma').textContent = accion === 'firmar' ? 'Firmar permiso' : (accion === 'devolver' ? 'Devolver permiso' : 'Rechazar permiso');
    document.getElementById('cajaFirmaBandeja').classList.toggle('hidden', accion !== 'firmar');
    document.getElementById('capturaFotoBandeja').parentElement.classList.toggle('hidden', accion !== 'firmar');
    document.getElementById('cajaMotivo').classList.toggle('hidden', accion === 'firmar');
...
function cerrarModalFirma() { document.getElementById('modalFirmarPermiso').classList.add('hidden'); }
...
document.getElementById('btnConfirmarAccion').addEventListener('click', async () => {
    ...
    if (accionActual === 'firmar') {
        url = rol === 'reemplazo' ? '/chvb/public/api/permisos_firmar_reemplazo.php' : '/chvb/public/api/permisos_firmar_jefe.php';
        ...
        if (usarGuardada && usarGuardada.checked) {
            ...
            error.textContent = 'Debes firmar antes de continuar.'; error.classList.remove('hidden'); return;
        } else {
            formData.append('firma_base64', canvasBandeja.obtenerDataURL());
        }
        if (capturaFotoBandeja.tieneFoto()) formData.append('foto_base64', capturaFotoBandeja.obtenerDataURL());
    } else if (accionActual === 'devolver') {
        url = '/chvb/public/api/permisos_devolver.php';
        const motivo = document.getElementById('motivoBandeja').value.trim();
        if (!motivo) { error.textContent = 'Indica el motivo.'; error.classList.remove('hidden'); return; }
        formData.append('motivo', motivo);
    } else {
        url = '/chvb/public/api/permisos_rechazar.php';
        const motivo = document.getElementById('motivoBandeja').value.trim();
        if (!motivo) { error.textContent = 'Indica el motivo.'; error.classList.remove('hidden'); return; }
        formData.append('motivo', motivo);
    }
