...
$empleados = $busqueda !== '' ? EmpleadoModel::listar($busqueda) : [];
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<div class="flex">
    ...
    <div class="flex-1">
        ...
        <?php if ($busqueda === ''): ?>
            <p class="text-sm text-gray-400">Escribe una cédula o nombre para buscar un empleado.</p>
                ...
                <?php foreach ($empleados as $emp): ?>
                    ...
                    <div class="bg-white rounded-lg shadow p-4 flex justify-between items-center">
                        <div>
                            ...
                            <?php if ($acceso): ?>
                                <span class="text-xs px-2 py-0.5 rounded <?= $acceso['activo'] ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600' ?>">
                        ...
                        <div class="flex gap-2">
                            <button onclick="abrirModalPin('<?= $emp['cedula'] ?>', '<?= htmlspecialchars($emp['nombre'], ENT_QUOTES) ?>')"
                                class="text-sm bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded">
                                <?= $acceso ? 'Resetear PIN' : 'Crear acceso' ?>
                            </button>
                            <?php if ($acceso && $acceso['activo']): ?>
                                <button onclick="revocarAcceso('<?= $emp['cedula'] ?>')"
                                    class="text-sm border border-gray-300 text-gray-600 hover:bg-gray-50 px-3 py-1.5 rounded">
                                    Revocar
                                </button>
                            <?php elseif ($acceso && !$acceso['activo']): ?>
                                <button onclick="reactivarAcceso('<?= $emp['cedula'] ?>')"
                                    class="text-sm border border-green-300 text-green-600 hover:bg-green-50 px-3 py-1.5 rounded">
                                    Reactivar
                                </button>
                            <?php endif; ?>
                        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg w-full max-w-sm">
            ...
            <form id="formPin" class="p-6 space-y-4">
                ...
                <div class="flex gap-2">
                    ...
                    <button type="submit" class="flex-1 bg-red-600 hover:bg-red-700 text-white rounded py-2">Guardar</button>
                </div>
            </form>
        </div>
    </div>
