                        ...
                        <tr class="<?= $cumple['cumple'] ? 'bg-yellow-50' : '' ?>">
                            <td class="px-4 py-3 font-medium text-gray-800">
                                <div class="flex items-center gap-2">
                                    ...
                                    <?php else: ?>
                                        <span class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs">👤</span>
                                    <?php endif; ?>
                                    <span>
                            ...
                            <td class="px-4 py-3 relative">
                                ...
                                <div id="menu-<?= $emp['cedula'] ?>" class="hidden absolute right-4 z-10 bg-white border rounded-xl shadow-md w-44 text-sm overflow-hidden">
                                    <button onclick="abrirModalVer('<?= $emp['cedula'] ?>')" class="block w-full text-left px-4 py-2 hover:bg-gray-50">Ver empleado</button>
                                    <button onclick="abrirModalEditar('<?= $emp['cedula'] ?>')" class="block w-full text-left px-4 py-2 hover:bg-gray-50">Editar</button>
...
<div id="modalVer" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div class="px-6 py-4 border-b flex justify-between items-center">
            ...
            <button onclick="document.getElementById('modalVer').classList.add('hidden')" class="text-gray-400 hover:text-gray-700">✕</button>
        </div>
...
<div id="modalEditar" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div class="px-6 py-4 border-b flex justify-between items-center">
            ...
            <button onclick="document.getElementById('modalEditar').classList.add('hidden')" class="text-gray-400 hover:text-gray-700">✕</button>
        </div>
        ...
        <form id="formEditar" class="p-6 space-y-4">
            ...
            <div class="flex items-center gap-3">
                <img id="editFotoActual" src="" class="w-14 h-14 rounded-full object-cover border border-gray-200 hidden">
                <span id="editFotoPlaceholder" class="w-14 h-14 rounded-full bg-gray-200 flex items-center justify-center text-xl">👤</span>
                <div class="flex-1">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Cambiar foto (opcional)</label>
                    <input type="file" name="foto" accept="image/*" class="w-full border border-gray-300 rounded px-3 py-2 text-sm">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="cedula" id="editCedula" required maxlength="10"
                    class="w-full border border-gray-300 rounded px-3 py-2">
                <p class="text-xs text-gray-400 mt-1">Cambiar la cédula renombra la carpeta física y todos sus documentos. Úsalo con cuidado.</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Cargo</label>
                <select name="cargo" id="editCargo" required class="w-full border border-gray-300 rounded px-3 py-2">
                    <?php foreach (EmpleadoController::$cargosValidos as $c): ?>
                        <option value="<?= htmlspecialchars($c) ?>"><?= htmlspecialchars($c) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Tipo de contrato</label>
                <select name="tipo_de_contrato" id="editContrato" required class="w-full border border-gray-300 rounded px-3 py-2">
                    <option value="fijo">Fijo</option>
                    <option value="indefinido">Indefinido</option>
                    <option value="ops">OPS</option>
                </select>
