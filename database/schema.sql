CREATE DATABASE IF NOT EXISTS chvb
...
CREATE TABLE IF NOT EXISTS empleados (
  ...
  foto VARCHAR(255) NULL COMMENT 'Ruta relativa dentro de uploads/, ej: hv_{cedula}/perfil/foto.jpg',
...
CREATE TABLE IF NOT EXISTS bolsillos (
  ...
  alarma_tipo ENUM('1m','2m','6m','1a','custom') NULL,
  alarma_valor INT NULL COMMENT 'Cantidad numérica para alarma personalizada',
  alarma_unidad ENUM('dias','meses','anios') NULL COMMENT 'Unidad para alarma personalizada',
  alarma_fecha_inicio DATE NULL COMMENT 'Fecha desde la cual se cuenta el plazo',
  alarma_fecha DATE NULL COMMENT 'Fecha calculada en que vence la alarma',
  alarma_dias_aviso INT NULL DEFAULT 35 COMMENT 'Días de anticipación para la alerta amarilla (15 si el tipo es 1m)',
...
-- ---------------------------------------------------------------------
-- Tabla: usuarios (SuperAdmin / Auxiliar / Teniente - Talento Humano)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Tabla: usuarios_empleados (login del empleado, PIN 4 dígitos)
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS usuarios_empleados (
  id INT AUTO_INCREMENT PRIMARY KEY,
  cedula VARCHAR(10) NOT NULL UNIQUE,
  password_hash VARCHAR(255) NOT NULL COMMENT 'hash del PIN de 4 digitos (validación de login)',
  pin_encriptado VARCHAR(255) NULL COMMENT 'PIN cifrado reversible, visible solo para superadmin/auxiliar vía ícono de ojo',
...
-- ---------------------------------------------------------------------
-- Tabla: notificaciones (acciones de auxiliares, revisadas por el superadmin)
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS notificaciones (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL COMMENT 'quien hizo el cambio (el auxiliar u otro rol no-superadmin)',
  usuario_nombre VARCHAR(50) NOT NULL,
  cedula_empleado VARCHAR(10) NOT NULL,
  campo VARCHAR(50) NOT NULL COMMENT 'nombre, cargo, correo, estado, es_bombero_integral, fecha_nacimiento, foto, creacion, documento',
